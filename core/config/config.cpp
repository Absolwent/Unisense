...
void config::get_Unisense_folder() {
	CHAR documents[500];
	HRESULT get_documents = SHGetFolderPath(NULL, CSIDL_PERSONAL, NULL, SHGFP_TYPE_CURRENT, documents);

	if (get_documents != S_OK) Unisense_config_folder = std::string("Unisense");							// Use current path if error
	else                       Unisense_config_folder = std::string(documents) + std::string("\\Unisense");	// Use documents folder if possible

	DWORD exitst = GetFileAttributesA(Unisense_config_folder.c_str());
	if (exitst == INVALID_FILE_ATTRIBUTES) {			// Path does not exist
		if (_mkdir(Unisense_config_folder.c_str()) == -1)
			throw std::runtime_error("Failed to create Unisense folder!");
		exitst = GetFileAttributesA(Unisense_config_folder.c_str());	// Get path again
	}
	...
	const std::string config_folder = Unisense_config_folder + "\\config";
...
void config::refresh_list() {
	...
	get_Unisense_folder();
	const std::string path = Unisense_config_folder + "\\config";
